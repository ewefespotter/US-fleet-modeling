...
### ORDER - run EV_Volumes_Clean, Historical, Scrap, Future
## Find some way to integrate Repeal and ACCII and redo plots --> national so can include manufac 
## update recycling and manufacturing facilities
...
## To do
## check 500 kg thing--> check chemistries and states for recycling and manufac----maybe do 2 manufac scens
## check on scale up is 2028 best year
## run state by state manufacturing and recycling



## Long term
## better projection of changes in manufacturing w overall Gwh and scenarios of change in chem
## current modeling looks at available mins from each state assuming all get recycled equally - manufacturing scrap is national
...
EVLIB_Flows <- read_csv("/Users/elsawefes-potter/Documents/Critical_Minerals_Pablo/EVLIB_Flows_detail_Repeal.csv")
EV_Flows <- read_csv("/Users/elsawefes-potter/Documents/Critical_Minerals_Pablo/ClosedLoop_AddRetire_byStateSegment_Repeal.csv") %>%
...
EVLIB_Flows_CA <- read_csv("/Users/elsawefes-potter/Documents/Critical_Minerals_Pablo/Canada-EVLIB_Flows_detail_Repeal.csv") 
EV_Flows_CA <- read_csv("/Users/elsawefes-potter/Documents/Critical_Minerals_Pablo/Canada-ClosedLoop_AddRetire_byStateSegment_Repeal.csv") %>%
  select(State, Segment, Year, add_BEV, add_PHEV) %>%
  group_by(State, Segment, Year) %>% summarise(add_BEV = sum(add_BEV, na.rm = TRUE), add_PHEV = sum(add_PHEV, na.rm = TRUE)) %>%
  rename(BEV = add_BEV, PHEV = add_PHEV) %>%
  pivot_longer(cols = c(BEV, PHEV),
...
batt_cap_projection <- batt_cap_projection %>%
  left_join(delay_scrap_proj_tonnes,by = "Sale_Year") %>%
  left_join(delay_prod_proj_Gwh_state,by = c("Year")) %>%
  select(
    Sale_Year,
    State_Province,
    Segment,
...
batt_cap_15 <- batt_cap_15 %>% 
  left_join(delay_scrap_15_tonnes, by = "Sale_Year") %>% 
  left_join(delay_prod_15_Gwh_state, by = c("Year")) %>% 
  select(
    Sale_Year, 
    State_Province, 
    Segment, 
...
capacity_chem_scenarios <- function(batt_cap_df, chem_df, mineral_intensity, future_recycle_type_collection) {
  ...
  Available_Recycling_Capacity <- Delay_US_CA_Recycle %>% 
    ...
    mutate(
      ...
      Unused_Refining = pmax(Refining_MT - pmin((Batt_Mass_MT + Scrap_tonnes)/Full_Recycle,1)*Full_Recycle,0), ## percent of full recycle used--> some of it
...
## Assuming all gets recovered- not necessairly the case at first
## should hold the minerals available --> applied the recycling to it already --> just have one manufac tho
...
ggplot(
  ...
  labs(
    title = "Delayed Openings - North America Available Recycled Minerals Until 2035 by Mineral",
    x = "Year",
...
ggplot(
  ...
  facet_wrap(~ Mineral, scales = "free_y") +
  labs(
    title = "Delayed Openings - Cumulative North America Minerals Lost to Lack of Recovery Standards",
    x = "Year",
...
ggplot(needed_cap_long, aes(
  ...
  labs(
    title = "Delayed Openings - North America Black Mass and Refining Capacity Needed Yearly Until 2050",
    x = "Year",
...
ggplot(export_lost, aes(
  ...
  facet_wrap(~ Mineral, scales = "free_y") +
  labs(
    title = "Delayed Openings - Exported Mass of Battery Minerals Each Year",
    x = "Year",
